...
const getList = async (start, count) => {
    const sql = "SELECT B.id AS id, B.title AS title, B.created_at AS createdAT, B.last_updated As lastUpdated" +
                "A.displayname As displayName FROM users AS A INNER JOIN articles AS B ON A.id = B.author" +
...
const getByID = async (id) => {
    const sql = "SELECT B.id AS id, B.title AS title, B.content AS content, B.created_at AS createdAT, B.last_updated As lastUpdated" +
                "A.displayname As displayName FROM users AS A INNER JOIN articles AS B ON A.id = B.author" +
...
const getByIdAndAuthor = async (id, author) => {
    const sql = "SELECT title, content, author, created_at AS createdAt, last_updated As lastUpdated" +
                "FROM articles WHERE id = ? AND author = ? AND is_active =1 AND is_deleted = =";
...
const create = async (title, content, author) => {
    const sql = 'INSERT INTO articles VALUES (DEFAULT, ?, ?, ?, DEFAULT, DEFAULT, DEFAULT, DEFAULT)';
...
const update = async (id, title, content) => {
    ...
    await runQuery(sql, [id, title, content]);
};

const remove = async id => {
    const sql = 'UPDATE articles SET is_deleted = 1 WHRER id = ?';
